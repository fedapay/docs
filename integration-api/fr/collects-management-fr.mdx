...
<Steps>
  <Step title="Requête de Création d'un Dépôt">
    ...
    Un exemple de code est disponible pour simplifier la création d’un dépôt via l'API. Assurez-vous de remplacer **VOTRE_CLE_API_PRIVEE** par votre clé privée sandbox ou live.
    </Info>
  ...
  <Step title="Envoi du Dépôt">
    ...
    Utilisez le code PHP, Node.js ou Ruby pour envoyer le dépôt en remplaçant **VOTRE_CLE_API_PRIVEE** par votre clé privée.
    </Info>
  ...
  <Step title="Récupération des Détails d'un Dépôt">
    ...
    <Info>
    [Code API pour récupérer les détails d'un dépôt](/api-reference/payouts/update-start)
