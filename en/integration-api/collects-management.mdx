...
<Steps>
  ...
  <Step title="Trigger the Payment">
    Once you have the payment token, you need to send a request to the FedaPay API to process the payment. The request is made using one of the specific [payment methods ](/en/payment-methods)
...
<Warning>
  **Important Notes :**
  ...
  Replace **METHODE_PAIEMENT** with the chosen [payment method](/en/payment-methods#available-no-redirect-payment-methods) (e.g., **mtn_benin**, **moov_benin**, etc.).
...
<Note>
  **Note**: Payments without redirection do not support all operators. Refer to the [Payment Methods](/en/payment-methods) section for more details on supported methods.
</Note>
...
* **Implement a webhook** to receive automatic notifications. [See the Webhooks section for more details.](/en/integration-api/webhooks).
