...
Bonne nouvelle : TAP & GO est déjà disponible dans votre tableau de bord. Il vous suffit d’activer la fonctionnalité. Pas besoin d’installer une nouvelle application. Vous pouvez ensuite encaisser  via QR ou USSD, et même commander un support(Kit ou carte) de paiement si besoin.
...
Oui. Comme pour toutes les transactions traitées via FedaPay, une commission standard de 1,6 % est prélevée automatiquement sur chaque paiement reçu via Mobile Money (MTN Benin, Moov Benin et Celtis).
