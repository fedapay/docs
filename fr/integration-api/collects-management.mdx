  ...
  <Step title="Initialisation de la Collecte : Création d'une Requête">
    ...
    * **customer** : le client concerné par la collecte
    ...
    <CodeGroup>
      ...
      ```javascript NodeJs highlight={9,18}
...
<Steps>
  ...
  <Step title="Déclencher le paiement">
    Une fois que vous avez le token de paiement, vous devez envoyer une requête à l'API FedaPay pour traiter le paiement. La requête se fait en utilisant une des [méthodes de paiement](/fr/payment-methods) spécifiques
...
<Warning>
  **Attention :**
  ...
  Remplacez **METHODE_PAIEMENT** par la [méthode de paiement](/fr/payment-methods#modes-de-paiement-disponibles-sans-redirection) choisie (par exemple **mtn_open**, **moov**, etc.).
  ...
  **_Important_**\_ : Veillez à tester minutieusement vos intégrations dans l'environnement **sandbox** avant de les déployer en production\_
</Warning>
...
<Note>
  **Remarque** : Le paiement sans redirection ne prend pas en charge tous les opérateurs. Consulter la section [Méthodes de Paiement](/fr/payment-methods) pour en savoir un peu plus.
</Note>
...
* **Implémentez un webhook** pour recevoir des notifications automatiques. [Consultez la section Webhooks pour plus de détails](/fr/integration-api/webhooks).
